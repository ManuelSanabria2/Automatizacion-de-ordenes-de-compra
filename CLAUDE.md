...
- **Security model:** RLS on every table, policies only for `authenticated` (single-tenant); `anon` has no access and cannot execute the numbering RPC.
...
- **Name matching cascade:** `alias_productos` exact match → fuzzy (rapidfuzz) against `productos_empresa` → Gemini suggestion only as last resort (keep token use minimal) → manual. User corrections in the review screen create/update aliases.
